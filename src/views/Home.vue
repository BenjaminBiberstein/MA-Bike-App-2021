<template>
  <ion-page>
      ...
      <ion-toolbar color="danger">
        <ion-grid>
          <ion-row>
            ...
            <ion-text  v-if="userLoggeIn">Logged in</ion-text>
          </ion-row>
        </ion-grid>
      </ion-toolbar>
    </ion-header>   
    <ion-content :fullscreen="true" class="ion-text-center">
      <h2>Bike-App</h2>
      <template v-if="!userLoggeIn">
        <app-login v-if="UserLogin"></app-login>
        <app-register v-else ></app-register>
        
        <ion-toolbar>
            <div>
              <ion-button  size="small" color="secondary" shape="round"  v-if="!UserLogin" @click.prevent="ChangeUser">
                If you have an accout Login here
              </ion-button>
              <ion-button  size="small" color="secondary" shape="round" v-else @click.prevent="ChangeUser">
...
export default defineComponent({
  ...
  methods: {
    ChangeUser() {
      return this.UserLogin = !this.UserLogin
    },
    
  },
  computed: {
    ...mapState(['userLoggeIn'])
  },
});
</script>
